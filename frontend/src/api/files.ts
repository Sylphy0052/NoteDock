...
// Upload file
export async function uploadFile(file: File, noteId?: number): Promise<FileUploadResponse> {
...
// Attach file to note
export async function attachFileToNote(fileId: number, noteId: number): Promise<MessageResponse> {
  const { data } = await apiClient.post<MessageResponse>(`/notes/${noteId}/files/${fileId}`)
  return data
}

// Detach file from note
export async function detachFileFromNote(fileId: number, noteId: number): Promise<MessageResponse> {
  const { data } = await apiClient.delete<MessageResponse>(`/notes/${noteId}/files/${fileId}`)
  return data
}
