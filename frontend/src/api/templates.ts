 ...
 * Update a template
 */
export async function updateTemplate(templateId: number, data: TemplateUpdate): Promise<Template> {
...
export async function deleteTemplate(templateId: number): Promise<{ message: string }> {
  const response = await apiClient.delete<{ message: string }>(`/templates/${templateId}`)
  return response.data
}
